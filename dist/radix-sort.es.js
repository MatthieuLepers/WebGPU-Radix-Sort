class P {
  ...
  shaderModules = {};
  constructor(e) {
    this.options = e, Object.keys(e).forEach((t) => {
...
function f(o, e) {
  ...
  if (e > o.limits.maxComputeWorkgroupsPerDimension) {
    const i = Math.floor(Math.sqrt(e)), r = Math.ceil(e / i);
    t.x = i, t.y = r;
  }
  return t;
}
function d({ device: o, label: e, data: t, usage: i = 0 }) {
  const r = o.createBuffer({
    label: e,
    usage: i,
    size: t.length * 4,
    mappedAtCreation: !0
  });
  return new Uint32Array(r.getMappedRange()).set(t), r.unmap(), r;
}
function G(o, e) {
  const t = Math.min(8192, e.size), i = Math.ceil(e.size / t), r = o.createTexture({
    size: {
      width: t,
      height: i
    },
  ...
  }), s = o.createCommandEncoder();
  return s.copyBufferToTexture({ buffer: e }, { texture: r }, [r.width, r.height, r.depthOrArrayLayers]), o.queue.submit([s.finish()]), r;
}
const m = (o) => o.split(`
...
class v extends P {
  ...
  constructor({
    device: e,
    count: t,
    workgroupSize: i = { x: 16, y: 16 },
    data: r,
    avoidBankConflicts: s = !1
  }) {
    if (super({ device: e, count: t, workgroupSize: i }), Math.log2(this.threadsPerWorkgroup) % 1 !== 0)
    ...
    this.shaderModules.prefixSum = this.device.createShaderModule({
      label: "prefix-sum",
      code: s ? k : D
    }), this.createPassRecursive(r, t);
  }
  createPassRecursive(e, t) {
    const i = Math.ceil(t / this.itemsPerWorkgroup), r = f(this.device, i), s = this.device.createBuffer({
      ...
      bindGroupLayouts: [u]
    }), l = this.device.createComputePipeline({
    ...
    if (this.pipelines.push({ pipeline: l, bindGroup: a, dispatchSize: r }), i > 1) {
      this.createPassRecursive(s, i);
      const c = this.device.createComputePipeline({
      ...
      this.pipelines.push({ pipeline: c, bindGroup: a, dispatchSize: r });
    }
  }
  getDispatchChain() {
   ...
   */
  dispatch(e, t, i = 0) {
    this.pipelines.forEach(({ pipeline: r, bindGroup: s, dispatchSize: u }, a) => {
      e.setPipeline(r), e.setBindGroup(0, s), t ? e.dispatchWorkgroupsIndirect(t, i + a * 3 * 4) : e.dispatchWorkgroups(u.x, u.y, 1);
    });
  }
}
class p extends P {
  buffers = {};
  constructor(e) {
    super(e), this.start = e.start ?? 0, this.mode = e.mode ?? "full", this.buffers.result = e.result, this.buffers.original = e.original, this.buffers.isSorted = e.isSorted;
  }
  // Find the best dispatch size for each pass to minimize unused workgroups
  static findOptimalDispatchChain(e, t, i) {
    const r = i.x * i.y, s = [];
    do {
      const u = Math.ceil(t / r), a = f(e, u);
      s.push(a.x, a.y, 1), t = u;
    } while (t > 1);
    return s;
  }
  dispatch(e, t, i = 0) {
    this.pipelines.forEach(({ pipeline: r, bindGroup: s }, u) => {
      const a = this.mode !== "reset" && (this.mode === "full" || u < this.pipelines.length - 1);
      e.setPipeline(r), e.setBindGroup(0, s), a && t ? e.dispatchWorkgroupsIndirect(t, i + u * 3 * 4) : e.dispatchWorkgroups(1, 1, 1);
    });
  }
}
class E extends P {
  ...
  initialDispatch = [];
  constructor(e) {
    super(e), this.bitCount = e.bitCount ?? 32, this.checkOrder = e.checkOrder ?? !1, this.avoidBankConflicts = e.avoidBankConflicts ?? !1;
  }
  get prefixBlockWorkgroupCount() {
  ...
  createPipelines() {
    this.#e();
    const e = this.#t();
    this.createResources(), this.#i(e), this.createCheckSortKernels(e);
    for (let t = 0; t < this.bitCount; t += 2) {
      const i = t % 4 === 0, r = this.getPassInData(i), s = this.getPassOutData(i), u = this.createBlockSumPipeline(r, t), a = this.createReorderPipeline(r, s, t);
      this.pipelines.push(u, a);
    }
  }
  #e() {
    ...
    this.kernels.prefixSum = t, this.buffers.prefixBlockSum = e;
  }
  #t() {
    const e = f(this.device, this.workgroupCount), t = this.kernels.prefixSum.getDispatchChain(), i = Math.min(this.count, this.threadsPerWorkgroup * 4), r = this.count - i, s = i - 1, u = p.findOptimalDispatchChain(this.device, i, this.workgroupSize), a = p.findOptimalDispatchChain(this.device, r, this.workgroupSize), n = [
      e.x,
      e.y,
      1,
      // Radix Sort + Reorder
    ...
    return this.dispatchSize = e, this.initialDispatch = n, {
      initialDispatch: n,
      dispatchSizesFull: a,
      checkSortFastCount: i,
      checkSortFullCount: r,
      startFull: s
    };
  }
  #i(e) {
    this.checkOrder && (this.buffers.dispatchSize = d({
      device: this.device,
  ...
  dispatch(e) {
    this.checkOrder ? this.#s(e) : this.#r(e);
  }
  #r(e) {
    for (let t = 0; t < this.bitCount / 2; t += 1) {
      const i = this.pipelines[t * 2], r = this.pipelines[t * 2 + 1];
      e.setPipeline(i.pipeline), e.setBindGroup(0, i.bindGroup), e.dispatchWorkgroups(this.dispatchSize.x, this.dispatchSize.y, 1), this.kernels.prefixSum.dispatch(e), e.setPipeline(r.pipeline), e.setBindGroup(0, r.bindGroup), e.dispatchWorkgroups(this.dispatchSize.x, this.dispatchSize.y, 1);
    }
  }
  #s(e) {
    this.kernels.checkSortReset.dispatch(e);
    for (let t = 0; t < this.bitCount / 2; t++) {
      const i = this.pipelines[t * 2], r = this.pipelines[t * 2 + 1];
      t % 2 == 0 && (this.kernels.checkSortFast.dispatch(e, this.buffers.dispatchSize, this.dispatchOffsets.checkSortFast), this.kernels.checkSortFull.dispatch(e, this.buffers.checkSortFullDispatchSize)), e.setPipeline(i.pipeline), e.setBindGroup(0, i.bindGroup), e.dispatchWorkgroupsIndirect(this.buffers.dispatchSize, this.dispatchOffsets.radixSort), this.kernels.prefixSum.dispatch(e, this.buffers.dispatchSize, this.dispatchOffsets.prefixSum), e.setPipeline(r.pipeline), e.setBindGroup(0, r.bindGroup), e.dispatchWorkgroupsIndirect(this.buffers.dispatchSize, this.dispatchOffsets.radixSort);
    }
  }
}
const x = (o) => (
...
class O extends p {
  outputs = [];
  constructor(e) {
    super(e), this.buffers.data = e.data.keys, this.createPassesRecursive(e.data, this.count);
  }
  createPassesRecursive(e, t, i = 0) {
    const r = Math.ceil(t / this.threadsPerWorkgroup), s = !i, u = r <= 1, a = `check-sort-${this.mode}-${i}`, n = u ? this.buffers.result : this.device.createBuffer({
      label: a,
      size: r * 4,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST
    ...
    }), _ = this.device.createPipelineLayout({
      bindGroupLayouts: [l]
    }), h = s ? this.start + t : t, g = s ? this.start : 0, b = this.device.createComputePipeline({
      layout: _,
      compute: {
        module: this.device.createShaderModule({
          label: a,
          code: U(s, u, this.mode, "buffer")
        }),
    ...
    this.outputs.push(n), this.pipelines.push({ pipeline: b, bindGroup: c }), u || this.createPassesRecursive({ keys: n }, r, i + 1);
  }
}
class M extends E {
  constructor(e) {
    super(e), this.localShuffle = e.localShuffle ?? !1, this.buffers.keys = e.data.keys, e.data.values && (this.buffers.values = e.data.values), this.createShaderModules(), this.createPipelines();
  }
  get hasValues() {
    return !!this.data.values;
  }
  get blockSumSource() {
    const e = this.localShuffle ? I : x("buffer");
    return this.hasValues ? e : m(e);
  }
  get reorderSource() {
    return this.hasValues ? R("buffer") : m(R("buffer"));
  }
  createResources() {
    this.buffers.tmpKeys = this.device.createBuffer({
      ...
      size: this.count * 4,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST
    }), this.hasValues && (this.buffers.tmpValues = this.device.createBuffer({
  ...
  getPassInData(e) {
    return {
      keys: e ? this.buffers.keys : this.buffers.tmpKeys,
      values: e ? this.buffers.values : this.buffers.tmpValues
    };
  }
  getPassOutData(e) {
    return {
      keys: e ? this.buffers.tmpKeys : this.buffers.keys,
      values: e ? this.buffers.tmpValues : this.buffers.values
    };
  }
  createBlockSumPipeline(e, t) {
    const i = this.device.createBindGroupLayout({
      label: "radix-sort-block-sum",
      entries: [
        {
          binding: 0,
  ...
  createCheckSortKernels(e) {
    if (!this.checkOrder)
      return;
    const { checkSortFastCount: t, checkSortFullCount: i, startFull: r } = e;
    this.kernels.checkSortFull = new O({
      mode: "full",
      device: this.device,
      data: this.data,
      ...
      start: r,
      workgroupSize: this.workgroupSize
    }), this.kernels.checkSortFast = new O({
      mode: "fast",
      device: this.device,
      data: this.data,
    ...
    const s = this.initialDispatch.length / 3;
    if (this.kernels.checkSortFast.threadsPerWorkgroup < this.kernels.checkSortFull.pipelines.length || this.kernels.checkSortFull.threadsPerWorkgroup < s) {
      console.warn("Warning: workgroup size is too small to enable check sort optimization, disabling..."), this.checkOrder = !1;
      return;
    }
    this.kernels.checkSortReset = new O({
      mode: "reset",
      device: this.device,
      data: this.data,
      original: this.buffers.originalDispatchSize,
      ...
      isSorted: this.buffers.isSorted,
      count: s,
      workgroupSize: f(this.device, s)
    });
  }
  createReorderPipeline(e, t, i) {
    const r = this.device.createBindGroupLayout({
      label: "radix-sort-reorder",
      entries: [
        {
          binding: 0,
...
class S extends p {
  ...
  outputs = [];
  constructor(e) {
    super(e), this.textures.read = e.data.texture, this.createPassesRecursive(e.data, this.count);
  }
  createPassesRecursive(e, t, i = 0) {
    const r = Math.ceil(t / this.threadsPerWorkgroup), s = !i, u = r <= 1, a = `check-sort-${this.mode}-${i}`, n = u ? this.buffers.result : this.device.createBuffer({
      label: a,
      size: r * 4,
      usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST
    }), l = G(this.device, n), c = this.device.createBindGroupLayout({
      entries: [
        {
          binding: 0,
          visibility: GPUShaderStage.COMPUTE,
      ...
      bindGroupLayouts: [c]
    }), g = s ? this.start + t : t, b = s ? this.start : 0, T = this.device.createComputePipeline({
      layout: h,
      compute: {
        module: this.device.createShaderModule({
          label: a,
          code: U(s, u, this.mode, "texture")
        }),
        entryPoint: this.mode == "reset" ? "reset" : "check_sort",
        constants: {
          ELEMENT_COUNT: g,
    ...
    this.outputs.push(l), this.pipelines.push({ pipeline: T, bindGroup: _ }), u || this.createPassesRecursive({ texture: l }, r, i + 1);
  }
}
class K extends E {
  textures = {};
  constructor(e) {
    super(e), this.textures.read = e.data.texture, this.createShaderModules(), this.createPipelines();
  }
  get hasValues() {
    return !0;
  }
  ...
  getPassInData(e) {
    return {
      texture: e ? this.textures.read : this.textures.write
    };
  }
  getPassOutData(e) {
    return {
      texture: e ? this.textures.write : this.textures.read
    };
  }
  createBlockSumPipeline(e, t) {
    const i = this.device.createBindGroupLayout({
  ...
  createCheckSortKernels(e) {
    if (!this.checkOrder)
      return;
    const { checkSortFastCount: t, checkSortFullCount: i, startFull: r } = e;
    this.kernels.checkSortFull = new S({
      mode: "full",
      device: this.device,
      data: this.data,
      result: this.buffers.dispatchSize,
      ...
      start: r,
      workgroupSize: this.workgroupSize
    }), this.kernels.checkSortFast = new S({
      mode: "fast",
      device: this.device,
      data: this.data,
      result: this.buffers.checkSortFullDispatchSize,
    ...
    this.kernels.checkSortReset = new S({
      mode: "reset",
      device: this.device,
      data: this.data,
      original: this.buffers.originalDispatchSize,
...
export {
  v as PrefixSumKernel,
  M as RadixSortBufferKernel,
  K as RadixSortTextureKernel
};
